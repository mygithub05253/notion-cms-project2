...
- [ ] 프로젝트 맞춤 컴포넌트 생성:
  ...
  - `FormField` - 커스텀 폼 필드 (Label + Input + Error)
...
**수락 기준**:
- 모든 공통 컴포넌트가 storybook 스타일로 독립적으로 사용 가능
...
- [ ] 더미 상태:
  - 로딩 상태 시각화
  - 에러 상태 시각화 (계정 오류, 네트워크 오류 등)

**수락 기준**:
- 모든 폼 요소가 정확한 레이아웃으로 배치됨
- 접근성 기준 충족 (label, aria 속성)
- 모바일과 데스크톱에서 가독성 우수
...
- [ ] 폼 검증 에러 표시:
  ...
  - 필드 강조 표시
- [ ] 더미 시뮬레이션:
  - 각 페이지에 에러/성공 상태 시뮬레이션 버튼 (개발 모드)
...
- ✅ Task 005: 로그인 페이지 UI
- ✅ Task 006: 대시보드 페이지 UI
- ✅ Task 007: 견적서 생성/수정 페이지 UI
- ✅ Task 008: 견적서 상세 페이지 (관리자)
- ✅ Task 009: 견적서 목록 페이지 (클라이언트)
- ✅ Task 010: 견적서 상세 페이지 (클라이언트)
- ✅ Task 011: 에러 처리 및 알림 UI
...
**버전**: 1.2
**현재 상태**: Phase 1 완료 ✅, Phase 2 시작 준비 완료
...
| **Phase 2** | 준비 완료 | 0% | 8개 Task 대기 중 (004-011) |
| **Phase 3** | 대기 | 0% | 8개 Task 대기 중 (012-019) |
| **Phase 4** | 대기 | 0% | 4개 Task 대기 중 (020-023) |
| **총계** | 진행 중 | 12% | 23개 Task 중 3개 완료 |

### 다음 액션 아이템

**즉시 진행 (우선순위 1순위)**:
1. **Task 004**: 공통 UI 컴포넌트 라이브러리 구현
   - shadcn/ui 기반 필수 컴포넌트 설치 및 커스터마이징
   - 프로젝트 맞춤 컴포넌트 개발 (InvoiceCard, InvoiceTable 등)
   - 더미 데이터로 컴포넌트 테스트

**의존성 순서**:
- Task 004 → Task 005-011 (페이지별 UI)
- Task 005-011 완료 → Phase 3 시작 가능
